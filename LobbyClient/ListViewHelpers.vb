Public Class ListViewHelpers
    ...
    Public Class ListViewComparer
        ...
        Public Function Compare(ByVal x As Object, ByVal y As _
            ...
            ' Is it an epoch?
            If IsNumeric(GameDetailsParser.GetEpochNumber(Split(string_x, " -> ")(0))) AndAlso GameDetailsParser.GetEpochNumber(Split(string_x, " -> ")(0)) <> 0 Then
                Dim split_x() As String = Split(string_x, " -> ")
                Dim split_y() As String = Split(string_y, " -> ")
                If split_x.Length = 1 Then
                    string_x = GameDetailsParser.GetEpochNumber(split_x(0)).ToString("D2") & "-" & GameDetailsParser.GetEpochNumber(split_x(0)).ToString("D2")
                Else
                    string_x = GameDetailsParser.GetEpochNumber(split_x(0)).ToString("D2") & "-" & GameDetailsParser.GetEpochNumber(split_x(1)).ToString("D2")
                End If
                If split_y.Length = 1 Then
                    string_y = GameDetailsParser.GetEpochNumber(split_y(0)).ToString("D2") & "-" & GameDetailsParser.GetEpochNumber(split_y(0)).ToString("D2")
                Else
                    string_y = GameDetailsParser.GetEpochNumber(split_y(0)).ToString("D2") & "-" & GameDetailsParser.GetEpochNumber(split_y(1)).ToString("D2")
                End If
            End If

            ' Compare them.
            If m_SortOrder = SortOrder.Ascending Then
                If IsNumeric(string_x) AndAlso IsNumeric(string_y) _
                    Then
